		...
		<div className='bg-secondary rounded-lg shadow'>
			<div className='p-4 text-center'>
				<div
					...
					style={{
						backgroundImage: `url("${user?.user?.bannerImg || "/banner.png"}")`,
					}}
				/>
				...
				<p className='text-info text-xs'>{user?.user?.connections?.length} {user?.user?.connections?.length==1?"connection":"connections"}</p>
			</div>
			...
			<div className='border-t border-base-100 p-4'>
				<Link to={`/profile/${user.user.username}`} className='text-sm font-semibold'>
					Visit your profile
				</Link>
			</div>
		</div>
